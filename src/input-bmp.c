...
static int ReadColorMap(FILE *, unsigned char[256][3], int, int, at_bool *, at_exception_type *);
static at_bool ReadChannelMasks(unsigned int *, Bitmap_Channel *, unsigned int);
static unsigned char *ReadImage(FILE *, int, int, unsigned char[256][3], int, int, int, int, at_bool, const Bitmap_Channel *, at_exception_type *);
...
at_bitmap input_bmp_reader(gchar * filename, at_input_opts_type * opts, at_msg_func msg_func, gpointer msg_data, gpointer user_data)
{
  ...
  if (!ReadOK(fd, magick, 2) ||
	  !(!strncmp(magick, "BA", 2) ||
		  ...
		  !strncmp(magick, "CP", 2)))
  {
	  LOG1("%s is not a valid BMP file", filename);
...
static at_bool ReadColorMap(FILE * fd, unsigned char buffer[256][3], int number, int size, 
	at_bool *Grey,
